-- #454846 - Ajuste no saldo em progressão por mérito

COMMENT ON COLUMN funcional.progressao.saldo IS 'Tempo, em dias, que excede os 12 meses de interstício mínimo desde a vigência da sua última progressão por mérito anterior a 01/01/2025.';
...
UPDATE funcional.progressao p SET saldo = x.saldo_em_dias FROM (
	...
	INNER JOIN (
		...
		INNER JOIN funcional.progressao ult ON (
	        ...
	        ult.id_tipo_progressao = atual.id_tipo_progressao
	        AND (ult.data_de_vigencia < atual.data_de_vigencia OR (ult.data_de_vigencia = atual.data_de_vigencia AND ult.data < atual.data))
	        AND ult.ativo = true
		)
		WHERE atual.ativo = TRUE AND atual.saldo > 0 AND atual.id_tipo_progressao = 6 AND ult.ativo = TRUE
	...
	WHERE p.ativo = TRUE
) AS x WHERE x.id_progressao = p.id_progressao AND p.saldo > 0;